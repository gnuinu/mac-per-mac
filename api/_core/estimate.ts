...
/**
 * 요청하신 대로 claude-sonnet-4-6이 기본값입니다. 더 정확한 추정이 필요하면
 * CLAUDE_MODEL 환경변수로 claude-sonnet-5 등으로 바꿀 수 있습니다.
 */
...
export async function handleEstimate({
  ...
  if (!env.ANTHROPIC_API_KEY) {
    return { status: 503, body: { error: 'ANTHROPIC_API_KEY가 설정되지 않았습니다' } };
  }

  // 키를 명시적으로 넘긴다. Workers에는 process.env가 없다.
  ...
  try {
    ...
    if (response.stop_reason === 'refusal') {
      return { status: 422, body: { error: '이 요청에는 답할 수 없습니다' } };
    }
    ...
    return { status: 502, body: { error: `추정에 실패했습니다: ${message}` } };
  }
}
