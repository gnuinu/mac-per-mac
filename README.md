...
"18.9km"라는 수치만으로는 얼마나 높은지 감이 오지 않는다. 그래서 쌓은 빅맥과
비유 대상을 **실제 비율대로** 나란히 세우고, 눈대중 기준으로 사람을 함께 그린다.

그림은 문자 격자로 적은 픽셀 스프라이트다 (`components/pixel/sprites.ts`).
`'#'`는 진한 톤, `'+'`는 옅은 톤, `'.'`는 빈 칸. 렌더링할 때 가로로 이어진 칸을
하나의 사각형으로 합쳐 SVG로 편다. 이미지 파일을 두지 않는 이유는 앱의 나머지와
같다 — 오프라인 동작, 번들 크기, 그리고 강조색 하나만 바꾸면 전부 따라오게 하려고.

두 가지 한계를 그림 안에서 정직하게 드러낸다.

- **사람이 사라지는 구간**: 롯데월드타워(555m)와 사람(1.73m)은 320:1이라
  한 그림에 실제 비율로 넣으면 사람이 1픽셀도 안 된다. 사람은 4픽셀 아래로
  작아지면 아예 그리지 않는다 — 점 하나만 남으면 오히려 오해를 부른다.
- **비율이 7배를 넘는 구간**: 작은 쪽이 안 보이므로 실제 비율을 포기하고
  기둥을 한 번 끊어 표시한 뒤, 범례에 "그림은 비율 축약"이라고 적는다.

도메인은 어떤 그림을 쓸지 모른다. `pickLandmark()`가 `id`만 돌려주고 스프라이트
선택은 UI가 한다.
