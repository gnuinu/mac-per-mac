...
```
React 18 + TypeScript + Vite · vite-plugin-pwa · Vercel
...
실시간 가격 조회를 로컬에서 쓰려면 `.env.example`을 `.env.local`로 복사한 뒤 키를
채우고 `vercel dev`로 띄운다. 키가 없어도 앱은 로컬 카탈로그만으로 정상 동작한다.
...
Vercel에서 `Cache-Control: no-cache`, 서비스워커에서 `NetworkFirst`로 잡아뒀다.
...
5. `.env.local`에 넣는다.
   ...
   ```
6. 배포 시에는 Vercel 프로젝트의 **Settings → Environment Variables**에 같은 두
   값을 등록한다.
...
2. `.env.local`에 `ANTHROPIC_API_KEY`를 넣는다. Vercel에도 같은 값을 등록한다.
...
- 강조색은 딥 그린 `#1F6F5C` 하나뿐. 나머지는 전부 무채색이다.
...
- 종이 질감은 이미지 없이 CSS `repeating-linear-gradient`로 낸다.
- 버거는 사진 대신 직접 그린 SVG. `currentColor`를 따르므로 어디에 놓든 주변 색을 받는다.
- 다크 모드는 `prefers-color-scheme` 자동 전환. 공유 카드는 라이트로 고정한다 —
  공유된 이미지가 보는 사람 설정에 따라 달라지면 곤란하므로.
